...
2.对于 ThreadLocalMap 中的每一个 Entry 来说，它的 key 是 ThreadLocal，同时这个 key 为一个 **弱引用**，当 ThreadLocal 没有强引用指向它的时候，下次垃圾回收必定将他回收，因为 value 是**强引用**，所以只要 ThreadLocalMap 存在不会被回收，又因为 ThreadLocalMap 是线程的成员变量，所以 value 会一直被保存到线程被销毁，这就有内存泄漏的问题。
...
##### 为什么ThreadLocalMap 中的 key 是弱引用？

> 只有被软引用所指向的对象，下次 GC 时这个对象必定会被回收。
...
##### 那么为什么 JDK 的实现者会使用弱引用呢？

弱引用保证了在这个 ThreadLocal 对象在没有强引用的时候，会被回收掉，这个 value 怎么办呢？JDK 实现者做了这样一个方法，在 ThreadLocal 对象的get，set 和 remove 方法时，会去清除那些 key 为 null 的 Entry，这样能够保持程序尽可能小的出现内存泄露问题（注意 **使用弱引用能尽量小的保证出现内存泄漏问题**），所以 JDK 选择使用弱引用是最正确的选择，**在使用 ThreadLocal 时，对于用不到的值，要尽量 remove 清除一下**。
...
##### **为什么 ThreadLocal 对象最好使用 private static 来修饰？**
 ...
 **key 使用弱引用解决了 ThreadLocal 对象本身的内存泄漏问题，使用 remove 方法解决了 Entry 的 value 的内存泄漏问题**。

##### 相关文章

[1] [ThreadLocal原理 - 废物大师兄 - 博客园 (cnblogs.com)](https://www.cnblogs.com/cjsblog/p/9773079.html)

[2] [深入理解ThreadLocal的原理及内存泄漏问题_goodluckwj的博客-CSDN博客](https://blog.csdn.net/qq_35634181/article/details/103996977)(这个博文前后矛盾，key 为弱引用)

[3] [ThreadLocal原理，内存泄漏问题，怎么解决 - Lucky小黄人^_^ - 博客园 (cnblogs.com)](https://www.cnblogs.com/hi3254014978/p/13720147.html)
...
[2] [Java Synchronised机制 | Jacks Blog (dreamtobe.cn)](https://blog.dreamtobe.cn/2015/11/13/java_synchronized/)
...
[1] [浅谈sleep、wait、yield、join区别_大龙的博客-CSDN博客_sleep yield](https://blog.csdn.net/ywlmsm1224811/article/details/94022647)
...
6. threadFactory：创建线程的工厂类，通常我们会自定义一个 threadFactory 设置线程的名称，这样我们就可以知道线程是由哪个工厂类创建的，可以快速定位排查问题。
...
- **「TIDYING」**：所有的任务都销毁了，workCount 为 0，线程池的状态在转换为 `TIDYING` 状态时，会执行钩子方法 terminated()。因为terminated()在ThreadPoolExecutor类中是空的，所以用户想在线程池变为 `TIDYING` 时进行相应的处理；可以通过重载 terminated() 函数来实现。
